...
def transcribe_with_assemblyai(job_id: str, file_path: str, log: Callable[[str], None]) -> Generator[None, None, Optional[str]]:
    ...
    log("Create transcript job ..."); yield None
    r = requests.post(
        f"{base}/transcript",
...
def summarize_with_gpt(job_id: str, text: str, log: Callable[[str], None]) -> Generator[None, None, Optional[str]]:
...
class AppState(rx.State):
    ...
    def run_job(self, job_id: str):
        ...
        try:
            ...
            self._save()
            summary = (
                (yield from summarize_with_gpt(job_id, j["transcript"], lambda m: self._append_log(job_id, m)))
    ...
    def regenerate_summary(self):
        ...
        try:
            summary = yield from summarize_with_gpt(jid, transcript, lambda m: self._append_log(jid, m))
            j["summary"] = summary or ""
            j["status"] = "completed"
            self._save()
            if summary:
...
def index() -> rx.Component:
    return rx.center(
        rx.vstack(
            rx.heading("FocusFlow — Jobs with Live Logs"),
            rx.text("Paste a Plaud share link or direct audio URL. Each job runs end-to-end with live updates."),
            rx.hstack(
                rx.input(
                    placeholder="https://share.plaud.ai/... or direct .mp3/.m4a/.wav",
                    value=AppState.audio_url,
                    on_change=AppState.set_audio_url,
                    width="600px",
                ),
                rx.button("Add Job", on_click=AppState.add_job),
            ),
            rx.hstack(
            ...
            rx.hstack(
                ...
                rx.box(
                    rx.vstack(
                        rx.foreach(
                            ...
                            lambda j: rx.button(
                                _job_row(j),
                                variant=rx.cond(AppState.current_job_id == j["id"], "solid", "soft"),
                                on_click=lambda: AppState.select_job(j["id"]),
                                width="100%",
                                justify="start",
                            ),
                        ),
                ...
                rx.box(
                    rx.cond(
                        AppState.current_job_id == "",
                        rx.center(rx.text("No job selected"), height="100%"),
                        rx.vstack(
                            rx.hstack(
                                rx.heading("Job Detail"),
                                rx.badge(AppState.current_status, variant="soft"),
                                rx.button("Re-summarize", size="2", on_click=AppState.regenerate_summary, variant="soft"),
                                rx.button("Retry", size="2", on_click=AppState.retry_current, variant="soft"),
                                rx.button("Delete", size="2", on_click=AppState.delete_current, color_scheme="red", variant="soft"),
                                spacing="3",
                            ),
                            rx.box(
                                rx.vstack(
                                    rx.foreach(AppState.current_logs, lambda line: rx.code(line, width="100%")),
                                    spacing="1",
                                    align="start",
                                ),
                                border="1px solid",
                            ...
                            rx.heading("Transcript", size="5"),
                            rx.box(
                                rx.text(
                                    rx.cond(
                                        AppState.current_transcript == "",
                                        "(empty)",
                                        AppState.current_transcript,
                                    )
                                ),
