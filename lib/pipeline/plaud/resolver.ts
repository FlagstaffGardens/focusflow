...
export async function resolvePlaudAudioUrl(
  ...
  if (token) {
    ...
    try {
      ...
      if (response.ok) {
        const raw = (await response.json()) as { data?: PlaudContent } | PlaudContent
        let content: PlaudContent = {}
        if ('data' in raw && raw.data && typeof raw.data === 'object') {
          content = raw.data as PlaudContent
        } else if (raw && typeof raw === 'object') {
          content = raw as PlaudContent
        }
        ...
        const keys: Array<keyof PlaudContent> = ['fileUrl', 'audioUrl', 'url']
        for (const key of keys) {
          const candidate = content[key]
          if (typeof candidate === 'string') {
            log(`Plaud content API resolved → ${candidate}`)
            return {
              audioUrl: candidate,
              meetingDate,
    ...
    try {
      ...
      if (nextDataMatch) {
        try {
          ...
          let pageProps: PlaudContent | undefined
          if (typeof data === 'object' && data !== null) {
            const props = (data as { props?: unknown }).props
            if (props && typeof props === 'object') {
              pageProps = (props as { pageProps?: PlaudContent }).pageProps
            }
          }
          if (!meetingDate) {
      ...
      for (const [, , candidate] of jsonUrls) {
        const candidateUrl = candidate.replace(/\\u002F/g, '/')
        if (/\.(mp3|m4a|wav)$/i.test(candidateUrl)) {
          log(`Plaud resolved (json) → ${candidateUrl}`)
          return { audioUrl: candidateUrl, meetingDate }
        }
      }
    } catch (error) {
...
function findAudioUrlInObject(obj: unknown): string | null {
