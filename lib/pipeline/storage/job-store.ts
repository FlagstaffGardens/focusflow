...
export class JobStore {
  ...
  private getDeadLetter(): DeadLetterEntry[] {
    try {
      ...
      return parsed.filter(isDeadLetterEntry)
    } catch {
      return []
    }
  }

  private saveDeadLetter(deadLetter: DeadLetterEntry[]): void {
...
function isDeadLetterEntry(entry: unknown): entry is DeadLetterEntry {
  return (
    typeof entry === 'object' &&
    entry !== null &&
    'job' in entry &&
    ...
    typeof (entry as { retryCount: unknown }).retryCount === 'number'
  )
}
